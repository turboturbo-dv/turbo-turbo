...
internal sealed class TurboDevPanel : MonoBehaviour
{
    ...
    private void OnGUI()
    {
        ...
        _rect = GUILayout.Window(GetInstanceID(), _rect, DrawWindow, "TurboTurbo Dev (F6)");
    }
    ...
    private interface ITweakSpec
    {
        string Key { get; }
        bool Changed { get; }
        void Reset();
        void Draw();
        string DumpLine(); // null when unchanged
    }
    ...
    private abstract class SpecBase<T>
    {
    ...
    private sealed class FloatSpec : SpecBase<float>, ITweakSpec
    {
        ...
        public string DumpLine() => Changed ? $"  {Key}: {Format(Get())}" : null;

        private static string Format(float v) => v.ToString("0.#####", CultureInfo.InvariantCulture);
    }

    private sealed class BoolSpec : SpecBase<bool>, ITweakSpec
    {
        ...
        public string DumpLine() => Changed ? $"  {Key}: {Get().ToString().ToLowerInvariant()}" : null;
    }

    private sealed class IntSpec : SpecBase<int>, ITweakSpec
    {
        ...
        public string DumpLine() => Changed ? $"  {Key}: {Get()}" : null;
    }
    ...
    private void BuildSmokeModelSections(EngineSimulationHost host)
    {
        ...
        if (models.Count > 0)
        {
            ...
            AddFloat(section, "sootOnsetLambda",
                "Lambda where soot starts forming (fueling richer than this smokes).",
                0.3f, 1.5f, false,
        ...
        AddFloat(global, "wetStackFillRate",
            ...
            0f, 0.5f, false, () => ExhaustSmokeModel.WetStackFillRate, v => ExhaustSmokeModel.WetStackFillRate = v);
        ...
        AddFloat(global, "wetStackBurnDemand",
            "Demand above which the wet-stack burn produces straw puffs.",
    ...
    private void BuildEmitterSections(EngineSimulationHost host)
    {
        ...
        if (shimmers.Count > 0)
        {
            ...
            AddFloat(section, "lifetime", "Particle lifetime in seconds (drives the shimmer decay envelope).",
